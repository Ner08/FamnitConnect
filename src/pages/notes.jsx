...
export default function Home({ match }) {
  ...
  React.useEffect(() => {
    ...
    const getNotesData = async () => {
      notesData = await getRequest(
        "https://famnit-connect.herokuapp.com/notes/list/" +
          uuid +
          "?subject_id=" +
          subject_id,
        source
      ...
      if (validURL(url)) {
        ...
        try {
          const response = await axios.post(
            "https://famnit-connect.herokuapp.com/notes/insert",
            {
              url: url,
              name: name,
              user_uuid: uuid,
        ...
        setSnackbarErrorText(
          "The url you have entered is not an url (hint: starts with https:// or http://)"
        );
      }
    }
  };
  ...
  const onLike = async (notes_id) => {
    try {
      const response = await axios.post(
        "https://famnit-connect.herokuapp.com/notes/like",
        {
          user_uuid: uuid,
          notes_id: notes_id,
        }
  ...
  const onDislike = async (notes_id) => {
    try {
      const response = await axios.post(
        "https://famnit-connect.herokuapp.com/notes/dislike",
        {
          user_uuid: uuid,
          notes_id: notes_id,
        }
      ...
      return (
        <div style={{ display: "inline-block" }}>
          <IconButton
            ...
            size="small"
            onClick={() => onLike(id)}
            style={{ marginTop: 6, marginLeft: 3, color: "green" }}
          >
            <ThumbUpAltIcon />
          </IconButton>
          <IconButton
          ...
          <IconButton size="small" style={{ marginTop: 6, color: "gray" }}>
            <ReportIcon />
          </IconButton>
        </div>
      );
      ...
      return (
        <div style={{ display: "inline-block" }}>
          <IconButton
            size="small"
            onClick={() => onLike(id)}
            style={{ marginTop: 6, marginLeft: 3, color: "#004c00" }}
          >
            <ThumbUpAltIcon />
          </IconButton>
          <IconButton
  ...
  const inGrid = () => {
    return notes.map(function (data) {
      if (data.likes > 0) {
        return (
          <Grid key={data.id} item xs={12} sm={12} md={6} lg={3} xl={2}>
            <div className={classes.paper}>
              ...
              >
                ...
                <Typography
                  ...
                  className={classes.facultyClass}
                >
                  {data.name}
                </Typography>
              </Button>
      ...
      return (
        <div className={classes.addNoteDiv}>
          <Paper elevation={5} className={classes.paperEnterNote}>
            <Typography style={{ marginTop: 20, color: "black" }}>
            ...
            <Typography style={{ marginTop: 2, color: "black", fontSize: 11 }}>
  ...
  return (
    <div className={classes.root}>
      ...
      <Paper className={classes.title}>
        {subject_name}
        <span className={classes.notesText}> NOTES</span>
      </Paper>
