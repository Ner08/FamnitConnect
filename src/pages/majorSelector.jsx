...
import { Container, Typography, FormControl, InputLabel, Select, MenuItem, CssBaseline, Button, Paper } from "@material-ui/core";
import { getRequest, postRequest } from "../utils/axiosRequests";
import { useDispatch } from "react-redux";
import { setMajorGrade, setMajors, setSubjetsSet, setforceReload, setforceReload2 } from "../redux/actions";
...
const useStyles = makeStyles(theme => ({
  ...
  paperSubjects:{
    height:'100%',
    backgroundColor: "#404040", overflowX: 'hidden',
  }
...
export default function Home() {
  ...
  const [majorsSelectWasFetched, setmajorsSelectWasFetched] = React.useState(false);
  ...
  React.useEffect(() => {
    ...
    const getMajorsData = async () => {
      subjectsData = await getRequest("https://famnit-connect.herokuapp.com/programs/programs/list", source);
  ...
  const handleChangeGrade = async event => {
    const gotSubjects = await getRequest("https://famnit-connect.herokuapp.com/programs/subjects/list?year=" + event.target.value + "&program_id=" + majorID)
  ...
  const inSelectMajor = () => {
    return majorsSelect.map(function (data, i) {
      return (
        <MenuItem key={data.id} value={data.label}>{data.label}</MenuItem>
      );
    });
  };
  ...
  const inSelectGrade = () => {
    ...
    for (let index = 1; index <= numOfYears; index++) {
      items.push(<MenuItem key={index} value={index}>{`${index}. Letnik`}</MenuItem>)
    }
    console.log(grade)
    return items
  };
  ...
  const inSelectSubjects = () => {
    return subjects.map(function (data, i) {
      return (
        <FormControlLabel
          key={i}
          control={<Checkbox style={{ color: "white" }} value={data.id} checked={checkedSubjects[i]} onChange={handleSubjectCheck(data.id)} />}
  ...
  const confirm = () => {
    ...
    if (selectedSubjects.length > 0) {
      ...
      const postSubjects = async () => {
        ...
        const response2 = postRequest("https://famnit-connect.herokuapp.com/programs/programs/add", body2)
    ...
    <Container component="main" maxWidth="xs">
      ...
      <div className={classes.root}>
        <Paper className={classes.paperRoot}>
          ...
          <InputLabel style={{color: '#5050505'}} id="select_major">Your Major</InputLabel>
            <Select
          ...
          <Typography component="h5" variant="h5" className={classes.textSelectGrade}>
            SELECT YOR GRADE
        </Typography>
          ...
          <FormControl className={classes.formControl}>
            <InputLabel
              ref={inputLabel} id="select_grade">
              Your Grade
        </InputLabel>
            <Select
          ...
          <Typography component="h5" variant="h5" className={classes.textSelectSubjects}>
            SELECT YOR SUBJECTS
        </Typography>
        <Scrollbars>
          ...
          <Button variant="contained" style={{ marginTop: 20, marginBottom: 20 }} onClick={confirm}>confirm</Button>
        </Paper>
      </div>
    </Container>
  );
